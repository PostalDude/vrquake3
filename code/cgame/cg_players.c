...
void TraceOut(const char *pOut, ...)
{
	...
	va_list marker;
	va_start(marker, pOut);
	
	p += vsprintf(p, pOut, marker);
	
	va_end(marker);

	strcpy(p, "\r\n");

	f = fopen("F:\\q3trace.log", "a+");
	if (f)
	{
		fputs(pStr, f);
		fclose(f);
...
static void CG_RunLerpFrame( clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, float speedScale ) {
	...
	//TraceOut("cg.time: %d   frameTime: %d   oldFrameTime: %d\n", cg.time, lf->frameTime, lf->oldFrameTime);
...
static void CG_PlayerAngles( centity_t *cent, vec3_t legs[3], vec3_t torso[3], vec3_t head[3] ) {
	...
	//*** Removed to remove drifting of weapon in hand. (not good in VR)
	...
	//*** Adjust lerp for collision with walls, preventing near clipping. Need to work on this...
	{
		trace_t trace;
		...
		VectorCopy(cent->lerpAngles, vangles);
		vangles[PITCH] = 0;
		AnglesToAxis(vangles, ang);
				
		//AngleVectors(cg.predictedPlayerState.viewangles, ang[0], ang[1], ang[2]);
		VectorMA(cent->lerpOrigin, 26, ang[0], view);		

		CG_Trace( &trace, cent->lerpOrigin, mins, maxs, view, cg.predictedPlayerState.clientNum, MASK_SOLID );
		
		if ( trace.fraction != 1.0 )
		{
			float disp = -(1.0 - trace.fraction) * 26;
			VectorMA( cent->lerpOrigin,  disp, ang[0], cent->lerpOrigin );
			VectorMA( cg.refdef.vieworg, disp, ang[0], cg.refdef.vieworg );
		}
	}
...
void CG_Player( centity_t *cent ) {
	...
	if ( myself) 
	{
		cg.snap->ps.pm_flags |= PMF_NO_JUMP_ANIM;
	...
	//*** We can remove Entities from the model By commenting this line for example! Useful to just keep legs in VR view for example!
	//if (!myself)
	...
	//
	//*** modify so that Mouse controls the parent (torso in this case)
//	if (!myself)
	{
...
/*	else
	{
		CG_GetPlayerWeaponOrigin(&legs, NULL, cent);
	}*/
}
