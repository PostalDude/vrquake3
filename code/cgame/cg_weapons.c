...
void CG_GetPlayerWeaponOrigin(refEntity_t *parent, playerState_t *ps, centity_t *cent)
{
	refEntity_t	gun;
	vec3_t		angles;
	weapon_t	weaponNum;
	...
	trap_R_LerpTag(&lerped, parent->hModel, parent->oldframe, parent->frame,
		1.0 - parent->backlerp, "tag_weapon");  //*** Tags in lerp for position. 
	
	VectorCopy(parent->origin, cent->gunOrigin);
	VectorMA(cent->gunOrigin, lerped.origin[0], parent->axis[0], cent->gunOrigin);
}
...
void CG_AddPlayerWeapon( refEntity_t *parent, playerState_t *ps, centity_t *cent, int team ) {
	...
	//*** GUN through wall ???
	{
		float   disp = 0.0f;
		trace_t trace;
		static vec3_t	mins = { -7, -7, -7 };
		static vec3_t	maxs = { 7, 7, 7 };
		vec3_t view;
		vec3_t ang[3];
		vec3_t vangles;
		...
		VectorMA(cent->lerpOrigin, 32, ang[0], view);

		CG_Trace( &trace, cent->lerpOrigin, mins, maxs, view, cg.predictedPlayerState.clientNum, MASK_SOLID );

		if ( trace.fraction != 1.0 )
		{
			disp = -(1.0 - trace.fraction) * 32;
		}
	
		VectorMA(gun.origin, lerped.origin[0]+disp, parent->axis[0], gun.origin);
	}
...
void CG_AddViewWeapon( playerState_t *ps ) {
	...
	VectorCopy(cent->gunOrigin, hand.origin); //***	
	...
	hand.renderfx =/* RF_DEPTHHACK | */ RF_FIRST_PERSON | RF_MINLIGHT; //***depthhack removed
