...
/*
===================
Field_VariableConsoleSizeDraw

Handles horizontal scrolling and cursor blinking
x, y, and width are in pixels
===================
*/
void Field_VariableConsoleSizeDraw( field_t *edit, int x, int y, int width, int size, qboolean showCursor,
		qboolean noColorEscape ) {
	int		len;
	int		drawLen;
	int		prestep;
	...
	// draw it
	if ( size == CONSOLE_SMALLCHAR_WIDTH ) {
		float	color[4];

		color[0] = color[1] = color[2] = color[3] = 1.0;
		SCR_DrawConsoleSmallStringExt( x, y, str, color, qfalse, noColorEscape );
	} else {
		// draw big string with drop shadow
		SCR_DrawBigString( x, y, str, 1.0, noColorEscape ); 
	}
	...
	if ( showCursor ) {
		...
		if ( size == CONSOLE_SMALLCHAR_WIDTH ) {
			SCR_DrawConsoleSmallChar( x + ( edit->cursor - prestep - i ) * size, y, cursorChar );
		} else {
			str[0] = cursorChar;
			str[1] = 0;
			SCR_DrawBigString( x + ( edit->cursor - prestep - i ) * size, y, str, 1.0, qfalse );
...
{
	Field_VariableConsoleSizeDraw( edit, x, y, width, CONSOLE_SMALLCHAR_WIDTH, showCursor, noColorEscape );
}
