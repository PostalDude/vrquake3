...
void RE_InterFrame( stereoFrame_t stereoFrame )
{
}
/*
=============
...
void SetupShaderDistortion(int eye, float VPX, float VPY, float VPW, float VPH)
{
	...
	glUniform2f(lenscenter,   x + (w + stereoCfg.XCenterOffset * vr_lenseoffset->value)*0.5f, y + h*0.5f);
	glUniform2f(screencenter, x + w*0.5f, y + h*0.5f);
	if (eye == 1)
		glUniform2f(offset, vr_viewofsx->value, vr_viewofsy->value);
	else
		glUniform2f(offset, -vr_viewofsx->value, vr_viewofsy->value);
