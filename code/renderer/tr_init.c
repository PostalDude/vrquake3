...
void R_Register( void ) 
{
	...
	if (OVRDetected)
		vr_ovrdetected = ri.Cvar_Get("vr_ovrdetected", "1", CVAR_LATCH | CVAR_CHEAT);
	else
	...
	r_flareSize = ri.Cvar_Get ("r_flareSize", "70", CVAR_CHEAT);
	...
	r_lodscale = ri.Cvar_Get( "r_lodscale", "1", CVAR_CHEAT );
	...
	//***
	r_oculusRift = ri.Cvar_Get("r_oculusRift", "0", CVAR_ARCHIVE );
...
refexport_t *GetRefAPI ( int apiVersion, refimport_t *rimp ) {
	...
	re.InitFBOs   = RE_InitFBOs;
	re.BeginFrame = RE_BeginFrame;
	re.InterFrame = RE_InterFrame;
