...
void R_SetupProjection(viewParms_t *dest, float zProj, qboolean computeFrustum)
{
	...
	float   a   = (float)HMD.HResolution / (2.0*(float)HMD.VResolution);  //*** Correct Aspect ratio is 2.0 *
	float   fov = 2.0*atan((float)HMD.VScreenSize/(2.0*(float)HMD.EyeToScreenDistance)) - vr_fovoffset->value;
	float   zfar = 1000.0;
	float   znear = 0.001;
...
/*
	...
	dest->projectionMatrix[10] = 0;
	dest->projectionMatrix[14] = 0;	
	
	dest->projectionMatrix[0] = 2 * zProj / width * frameMultiplier;
	dest->projectionMatrix[4] = 0;
	dest->projectionMatrix[8] = (xmax + xmin + 2 * stereoSep) / width; //*** We can hack width here to make the oculus rift width aspect ratio correction
